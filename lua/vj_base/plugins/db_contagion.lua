...
-----------------------------------------------*/
    ...
    -- ConVars --
    VJ.AddConVar("VJ_CON_AllowClimbing", 0, bit_bor(FCVAR_ARCHIVE, FCVAR_NOTIFY))
    VJ.AddConVar("VJ_CON_SprinterChance", 10, bit_bor(FCVAR_ARCHIVE, FCVAR_NOTIFY))
    VJ.AddConVar("VJ_CON_Panic", 0, bit_bor(FCVAR_ARCHIVE, FCVAR_NOTIFY))
    VJ.AddConVar("VJ_CON_BreakDoors", 0, bit_bor(FCVAR_ARCHIVE, FCVAR_NOTIFY))
    VJ.AddConVar("VJ_CON_Difficulty", 3, bit_bor(FCVAR_ARCHIVE, FCVAR_NOTIFY))
    VJ.AddConVar("VJ_CON_Headshot", 0, bit_bor(FCVAR_ARCHIVE, FCVAR_NOTIFY))
    VJ.AddConVar("VJ_CON_CorpseEffects", 1, bit_bor(FCVAR_ARCHIVE, FCVAR_NOTIFY))
    ...
    -- Map Spawner ConVars --
    VJ.AddConVar("VJ_CON_MapSpawner_Enabled", 1, bit_bor(FCVAR_ARCHIVE, FCVAR_NOTIFY))
    VJ.AddConVar("VJ_CON_MapSpawner_MaxZom", 80, bit_bor(FCVAR_ARCHIVE, FCVAR_NOTIFY))
    VJ.AddConVar("VJ_CON_MapSpawner_HordeCount", 35, bit_bor(FCVAR_ARCHIVE, FCVAR_NOTIFY))
    VJ.AddConVar("VJ_CON_MapSpawner_SpawnMax", 2000, bit_bor(FCVAR_ARCHIVE, FCVAR_NOTIFY))
    VJ.AddConVar("VJ_CON_MapSpawner_SpawnMin", 650, bit_bor(FCVAR_ARCHIVE, FCVAR_NOTIFY))
    VJ.AddConVar("VJ_CON_MapSpawner_HordeChance", 100, bit_bor(FCVAR_ARCHIVE, FCVAR_NOTIFY))
    VJ.AddConVar("VJ_CON_MapSpawner_HordeCooldownMin", 120, bit_bor(FCVAR_ARCHIVE, FCVAR_NOTIFY))
    VJ.AddConVar("VJ_CON_MapSpawner_HordeCooldownMax", 180, bit_bor(FCVAR_ARCHIVE, FCVAR_NOTIFY))
    VJ.AddConVar("VJ_CON_MapSpawner_DelayMin", 0.85, bit_bor(FCVAR_ARCHIVE, FCVAR_NOTIFY))
    VJ.AddConVar("VJ_CON_MapSpawner_DelayMax", 3, bit_bor(FCVAR_ARCHIVE, FCVAR_NOTIFY))
...
if CLIENT then
    hook.Add("PopulateToolMenu", "VJ_ADDTOMENU_CON", function()
        spawnmenu.AddToolMenuOption("DrVrej", "SNPC Configures", "Contagion - General", "Contagion - General", "", "", function(panel)
            panel:AddControl("Button", {Text = "#vjbase.menu.general.reset.everything", Command = "VJ_CON_ZombieOverlay 1\nVJ_CON_OldOverlay 0"})
            ...
            panel:AddControl("Button", {Text = "#vjbase.menu.general.reset.everything", Command = "VJ_CON_AllowClimbing 0\nVJ_CON_RunnerChance 10\nVJ_CON_Sprinters 0\nVJ_CON_BreakDoors 1\nVJ_CON_Damage 3\nVJ_CON_CorpseEffects 1"})
            ...
            difList:AddOption("Easy", {VJ_CON_Difficulty = "1"})
            difList:AddOption("Normal", {VJ_CON_Difficulty = "2"})
            difList:AddOption("Hard", {VJ_CON_Difficulty = "3"})
            difList:AddOption("Extreme", {VJ_CON_Difficulty = "4"})
            difList:AddOption("Nightmare", {VJ_CON_Difficulty = "5"})
            ...
            panel:CheckBox("Enable Panic! Mode?", "VJ_CON_Panic")
            ...
            panel:CheckBox("Enable Corpse FX & Decals?", "VJ_CON_CorpseEffects")
            panel:NumSlider("Sprinter Chance", "VJ_CON_SprinterChance", 1, 100, 0)
...
hook.Add("EntityRemoved", "VJ_CON_AddNodes", function(ent)
    if ent:GetClass() == "info_node" then
        table_insert(VJ_CON_NODEPOS, ent:GetPos())
    end
end)
            ...
            if IsValid(ent) then
                ...
                for _, v in pairs(ents.GetAll()) do
                    if (v:IsNPC() && v:GetClass() != "obj_vj_bullseye" or v:IsPlayer()) && !v:IsFlagSet(FL_NOTARGET) then
                        if string_find(v:GetClass(), "npc_vj_con_z") && v:GetClass() != "npc_vj_con_zcarrier" && v:GetClass() != "npc_vj_con_zriotbrute" && v:GetClass() != "npc_vj_con_zscreamer" then
...
function VJ_CON_ApplyCorpseEffects(ent, corpse)
    ...
    hook.Add("EntityTakeDamage", "VJ_CON_CorpseBleed", function(target, dmginfo)
        if target.CON_Corpse && !target.Dead && CurTime() > target.CON_Corpse_StartT && target:GetColor().a > 50 then
            ...
            //sound.Play("physics/flesh/flesh_impact_bullet" .. math_random(1,3) .. ".wav", dmginfo:GetDamagePosition(), 60, 100)

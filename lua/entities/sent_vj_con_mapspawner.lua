...
ENT.Zombie = {
    ...
    {class = "npc_vj_con_zworker", chance = 8},
    {class = "npc_vj_con_zmilitary", chance = 8},
    {class = "npc_vj_con_zriot", chance = 20},
    {class = "npc_vj_con_zcurtis", chance = 6},
    {class = "npc_vj_con_zdiego", chance = 6},
    {class = "npc_vj_con_zelijah", chance = 6},
    ...
    {class = "npc_vj_con_zriotsol", chance = 20},
}

ENT.BossZombie = {
    {class = "npc_vj_con_zcarrier", max = 1},
    {class = "npc_vj_con_zriotbrute", max = 1},
    {class = "npc_vj_con_zscreamer", max = 1},
}

ENT.HordeAlert = {
    "vj_contagion/zombies/shared/horde_alert01_r.wav",
    "vj_contagion/zombies/shared/horde_alert02_r.wav",
    "vj_contagion/zombies/shared/horde_alert03_r.wav"
}
---------------------------------------------------------------------------------------------------------------------------------------------
function ENT:Initialize()
    ...
    self.IsActivated = tobool(GetConVar("VJ_CON_MapSpawner_Enabled"):GetInt())
    self.CON_SpawnDistance = GetConVar("VJ_CON_MapSpawner_SpawnMax"):GetInt()
    self.CON_SpawnDistanceClose = GetConVar("VJ_CON_MapSpawner_SpawnMin"):GetInt()
    self.CON_HordeChance = GetConVar("VJ_CON_MapSpawner_HordeChance"):GetInt()
    self.CON_HordeCooldownMin = GetConVar("VJ_CON_MapSpawner_HordeCooldownMin"):GetInt()
    self.CON_HordeCooldownMax = GetConVar("VJ_CON_MapSpawner_HordeCooldownMax"):GetInt()
    self.CON_MaxZombie = GetConVar("VJ_CON_MapSpawner_MaxZom"):GetInt()
    self.CON_MaxHordeSpawn = GetConVar("VJ_CON_MapSpawner_HordeCount"):GetInt()
    ...
    self.NextBossZombieSpawnTime = CurTime() + math_random(4,20)
    self.NextHordeSpawnTime = CurTime() + math_rand(self.CON_HordeCooldownMin, self.CON_HordeCooldownMax)
...
function ENT:CheckEnemyDistance(ent, remove)
    ...
    if closestDist >= GetConVar("VJ_CON_MapSpawner_SpawnMax"):GetInt() + 1000 && !visible && !remove then
        SafeRemoveEntity(ent)
    end
end
---------------------------------------------------------------------------------------------------------------------------------------------
function ENT:Think()
    self.IsActivated = GetConVar("VJ_CON_MapSpawner_Enabled"):GetInt()
    if self.IsActivated then
        ...
        self.CON_MaxZombie = GetConVar("VJ_CON_MapSpawner_MaxZom"):GetInt()
        self.CON_MaxHordeSpawn = GetConVar("VJ_CON_MapSpawner_HordeCount"):GetInt()
        //self.AI_RefreshTime = GetConVar("VJ_CON_MapSpawner_RefreshRate"):GetInt()
        ...
        if CurTime() > self.NextZombieSpawnTime then
            ...
            self.NextZombieSpawnTime = CurTime() + math_rand(GetConVar("VJ_CON_MapSpawner_DelayMin"):GetInt(), GetConVar("VJ_CON_MapSpawner_DelayMax"):GetInt())
        end

        if GetConVar("VJ_CON_MapSpawner_Boss") == 1 then
            ...
            if CurTime() > self.NextBossZombieSpawnTime then
                ...
                self.NextBossZombieSpawnTime = CurTime() +math_rand(4,20)
            end
        end
                -- Spawns Hordes
        if CurTime() > self.NextHordeSpawnTime && math_random(1, self.CON_HordeChance) == 1 then
            ...
            local horde = VJ.PICK(self.HordeAlert)
            for _, v in ipairs(player.GetAll()) do
                v:ChatPrint("Incoming horde!")
                v:EmitSound(horde, 100, 100)
            end
            self.NextHordeSpawnTime = CurTime() +math_rand(self.CON_HordeCooldownMin, self.CON_HordeCooldownMax)
        end
    end
